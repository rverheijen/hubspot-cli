...
export function diffObjectBundles(local, remote) {
  const scalarChanges = [
    diffScalar('labels.singular', local.labels?.singular, remote.labels?.singular),
    diffScalar('labels.plural', local.labels?.plural, remote.labels?.plural),
    diffScalar('primaryDisplayProperty', local.primaryDisplayProperty, remote.primaryDisplayProperty),
  ].filter(Boolean);

  return {
    scalarChanges,
    requiredProperties: diffSet(local.requiredProperties, remote.requiredProperties),
