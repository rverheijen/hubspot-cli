...
export async function pullObject(target, baseDir, envName, manifest, client) {
  ...
  if (target === '--all') {
    ...
    for (const schema of schemas.results) {
      try {
        await saveObject(schema, dir, envName, manifest, client);
        ok++;
      } catch (e) {
...
async function saveObject(schema, dir, envName, manifest, client) {
  ...
  const bundle = {
    ...
    propertyGroups: (groups.results || []).map(g => ({
      name: g.name,
      label: g.label,
    ...
    properties: (properties.results || []).map(p => cleanProperty(p)),
  };
